...
builder.Services.AddHttpClient("rates", client =>
	{
   ...
   .ConfigureHttpClient((provider, client) => { }); // additional configuration
...
app.MapGet("/httpclientfactory", [EndpointSummary("Send requests using IHttpClientFactory")]
...
app.MapGet("/namedclient", [EndpointSummary("Send requests using named client")]
	async (IHttpClientFactory clientFactory) =>
	{
