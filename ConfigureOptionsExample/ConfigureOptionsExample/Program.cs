...
builder.Services.AddOptions<CurrencyOptions>()
   ...
   .Configure(x => x.Currencies = new[] { "GBP", "USD", "EUR" })
   .Configure<ILocalisationOptionsProvider>((opts, service) => { opts.Currencies = service.GetCurrencies(); });
...
builder.Services.Configure<LanguageOptions>(options =>
	options.Languages = new[] { "English", "French", "Spanish" });
...
public class CurrencyProvider : ILocalisationOptionsProvider
{
	...
	{
		// Load the settings from a database / remote API for example
		return new[] { "English", "French", "Spanish", "German" };
	}
}